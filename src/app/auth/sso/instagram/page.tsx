...
export default function InstagramSSOPopup() {
  ...
  const handleAuthorize = async () => {
    ...
    try {
      const res = await fetch("/api/auth/sso", {
        ...
        body: JSON.stringify({
          email: "sethu_insta@creato.in",
          provider: "instagram",
          displayName: "Sethu Insta Vlogs",
          handle: "sethu_insta"
        })
      });
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
      <div className="w-full max-w-sm bg-white rounded-xl shadow-md border border-slate-100 p-8 text-center space-y-6">
        ...
        <div className="space-y-1.5">
          <h2 className="text-lg font-bold text-slate-800">Instagram SSO Link</h2>
          <p className="text-xs text-slate-500">Authorize **Creato** to connect your account</p>
        </div>
        ...
        {loading ? (
          <div className="py-8 flex flex-col items-center justify-center gap-2">
            ...
            <span className="text-xs text-slate-400 font-semibold">Linking Meta Session...</span>
          </div>
        ) : (
          <div className="text-left space-y-4">
            <p className="text-xs text-slate-655 leading-relaxed">
              **Creato** is requesting access to import your public credentials:
            </p>
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-2.5 text-xs text-slate-700">
                ...
                <div>
                  <p className="font-bold">Instagram Username & Handle</p>
                  <p className="text-[10px] text-slate-455">Import handle `@sethu_insta` and profile picture.</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2.5 text-xs text-slate-700">
                <CheckCircle2 className="w-4 h-4 text-purple-600 mt-0.5 shrink-0" />
                <div>
                  <p className="font-bold">Instagram Graph Insights</p>
                  <p className="text-[10px] text-slate-455">Import follower count data and comment logs for dynamic ATI trust verification.</p>
                </div>
              </div>
            </div>
            ...
            <div className="pt-4 grid grid-cols-2 gap-3">
              <button
                ...
                className="py-2.5 px-4 border border-slate-200 text-slate-500 text-xs font-semibold rounded-lg hover:bg-slate-50 text-center cursor-pointer"
              >
                Cancel
              </button>
        ...
        <div className="text-[9px] text-slate-400 leading-normal border-t border-slate-100 pt-4 flex items-center gap-1.5 justify-center">
          ...
          <span>Meta Secure OAuth Verification Standard</span>
        </div>
      </div>
    </div>
  );
